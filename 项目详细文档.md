...
### 项目背景
本项目是一个面向青少年体态监测与篮球动作矫正的智能化系统，结合了现代Web技术与计算机视觉技术，为家长和教练提供专业的训练指导和体态分析服务。

### 核心价值
- **科学训练**: 基于AI的精确动作分析，提供专业的训练建议
- **体态监测**: 实时监测青少年体态发展，预防不良姿势
- **数据可视化**: 直观的图表展示训练进展和体态变化
- **多角色支持**: 家长和教练双视角，满足不同使用需求
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   前端Web应用    │    │   后端AI引擎     │    │   硬件设备层     │
│                │    │                │    │                │
│ React 19 + TS   │◄──►│ Python + CV     │◄──►│ 摄像头/云台     │
│ TailwindCSS    │    │ PyTorch + YOLO  │    │ USB-CAN设备    │
│ Zustand        │    │ OpenCV          │    │                │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
### 数据流架构
```
视频输入 → 姿态检测 → 角度计算 → 动作分析 → 结果展示
    ↓         ↓         ↓         ↓         ↓
  摄像头    3D Pose   关节角度   场景识别   Web界面
  文件      Estimation  计算      算法      可视化
```

### 模块化设计
- **前端模块**: 用户界面、数据展示、报告生成
- **AI引擎模块**: 姿态估计、目标检测、场景分析
- **硬件接口模块**: 摄像头控制、云台驱动、CAN通信
- **数据处理模块**: 数据存储、格式转换、结果输出
...
#### 核心框架
- **React 19**: 最新版本的React框架，支持并发特性和Suspense
- **TypeScript**: 静态类型检查，提高代码质量和开发效率
- **Vite**: 现代化构建工具，快速的开发服务器和构建性能
...
- **jsPDF**: PDF生成库，支持图文报告导出

### 后端技术栈

#### 核心框架
- **Python 3.8+**: 主要编程语言
- **OpenCV 4.7.0**: 计算机视觉核心库
- **PyTorch 1.12.1**: 深度学习框架
- **NumPy 1.23.5**: 数值计算库

#### AI模型与算法
- **3D Human Pose Estimation**: 三维姿态估计模型
- **YOLOv8**: 目标检测模型
- **Ultralytics**: YOLO模型的Python封装
...
│   ├── components/          # 通用组件
│   │   ├── ui/             # shadcn/ui组件
│   │   ├── AnglePanel.tsx  # 角度面板组件
│   │   ├── MetricsBar.tsx  # 指标条组件
│   │   ├── TrendCharts.tsx # 趋势图表组件
│   │   └── ...
│   ├── pages/              # 页面组件
│   │   ├── Login.tsx       # 登录页面
│   │   ├── ParentTabs.tsx  # 家长端页面
│   │   ├── CoachTabs.tsx   # 教练端页面
│   │   └── ...
│   ├── hooks/              # 自定义Hook
│   ├── lib/                # 工具库
